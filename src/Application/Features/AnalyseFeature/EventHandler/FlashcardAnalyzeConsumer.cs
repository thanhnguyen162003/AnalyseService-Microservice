...
namespace Application.Features.AnalyseFeature.EventHandler
{
    public class FlashcardAnalyzeConsumer(
        ...
        string topicName = "flashcard-analyze-topic",
        string groupId = "flashcard-analyze-group")
...
// private void CalculateAdvancedMetricsEnhanced()
   // {
   //     foreach (var userKvp in _userFlashcardAnalytics)
   //     {
   //         var userId = userKvp.Key;
   ...
   //         if (_userLearningPatterns.TryGetValue(userId, out var userPatterns))
   //         {
   //             CalculateCognitiveLoadIndex(flashcardAnalytics, userPatterns);
   //             AnalyzeLearningProgressRate(flashcardAnalytics, userPatterns);
   //             IdentifyDifficultFlashcards(flashcardAnalytics, userId);
   //             SegmentUserLearningBehavior(userId, flashcardAnalytics, userPatterns);
   //             CalculateOptimizationScore(userId, flashcardAnalytics, userPatterns);
   //         }
   //     }
   // }
   // private void CalculateCognitiveLoadIndex(ConcurrentDictionary<Guid, FlashcardAnalytics> flashcardAnalytics, UserLearningPatterns userPatterns)
   // {
   //     var recentSessions = _sessionAnalytics.Values
   //         .Where(s => s.UserId == userPatterns.UserId && s.EndTime >= DateTime.UtcNow.AddDays(-7))
   //         .ToList();
   //
   //     foreach (var session in recentSessions)
   //     {
   //         if (session.TotalTimeSpentMs > 30 * 60 * 1000) // > 30 phút
   //         {
   //             var flashcardsInSession = JsonConvert.DeserializeObject<List<Guid>>(session.FlashcardIds.ToString()) ?? new List<Guid>();
   //             var sessionFlashcardAnalytics = flashcardsInSession
   //                 .Where(id => flashcardAnalytics.ContainsKey(id))
   //                 .Select(id => flashcardAnalytics[id])
   //                 .ToList();
   //
   //             if (sessionFlashcardAnalytics.Count > 10)
   //             {
   //                 double initialAccuracy = sessionFlashcardAnalytics.Take(5).Average(a => a.AccuracyRate);
   //                 double laterAccuracy = sessionFlashcardAnalytics.Skip(sessionFlashcardAnalytics.Count - 5).Take(5).Average(a => a.AccuracyRate);
   //
   //                 if (laterAccuracy < initialAccuracy * 0.85)
   //                 {
   //                     foreach (var id in flashcardsInSession.Skip(flashcardsInSession.Count / 2))
   //                     {
   //                         if (flashcardAnalytics.TryGetValue(id, out var analytics))
   //                         {
   //                             analytics.CognitiveLoadIndex = Math.Min(1.0, (initialAccuracy - laterAccuracy) / initialAccuracy);
   //                         }
   //                     }
   //                 }
   //             }
   ...
   // }
   //
   // private void AnalyzeLearningProgressRate(ConcurrentDictionary<Guid, FlashcardAnalytics> flashcardAnalytics, UserLearningPatterns userPatterns)
   // {
   //     var learningRateData = new List<double>();
   //
   //     foreach (var analytics in flashcardAnalytics.Values)
   //     {
   //         if (analytics.ViewCount >= 3 && analytics.DailyViewCounts.Count >= 2)
   //         {
   //             var firstViewDate = analytics.DailyViewCounts.Keys.Min();
   //             var daysPassed = (DateTime.UtcNow - firstViewDate).TotalDays;
   //             if (daysPassed > 0)
   //             {
   //                 var progressRate = analytics.AccuracyRate / daysPassed;
   //                 if (!double.IsNaN(progressRate) && !double.IsInfinity(progressRate))
   //                 {
   //                     learningRateData.Add(progressRate);
   //                     analytics.LearningProgressRate = progressRate;
   //                 }
   //             }
   //         }
   //     }
   //
   //     if (learningRateData.Any())
   //     {
   //         userPatterns.AverageLearningProgressRate = learningRateData.Average();
   //         userPatterns.LearningRateHistory ??= new Dictionary<DateTime, double>();
   //         userPatterns.LearningRateHistory[DateTime.UtcNow.Date] = userPatterns.AverageLearningProgressRate.Value;
   //     }
   // }
   //
   // private void IdentifyDifficultFlashcards(ConcurrentDictionary<Guid, FlashcardAnalytics> flashcardAnalytics, Guid userId)
   // {
   //     var difficultFlashcards = new List<(Guid FlashcardId, double DifficultyScore)>();
   //
   //     foreach (var kvp in flashcardAnalytics)
   //     {
   //         var analytics = kvp.Value;
   //         double difficultyScore = 0;
   //
   //         if (analytics.AccuracyRate < 70 && analytics.ViewCount > 2)
   //             difficultyScore += (70 - analytics.AccuracyRate) / 70 * 5;
   //         if (analytics.AverageAnswerTimeMs > flashcardAnalytics.Values.Average(a => a.AverageAnswerTimeMs) * 1.5)
   //             difficultyScore += 3;
   //         if (analytics.RepetitionNumber > 3 && analytics.AccuracyRate < 80)
   //             difficultyScore += 4;
   //         if (analytics.ForgettingIndex > 75)
   //             difficultyScore += 3;
   //
   //         analytics.DifficultyScore = difficultyScore;
   //         if (difficultyScore > 5)
   //             difficultFlashcards.Add((kvp.Key, difficultyScore));
   //     }
   // }
   //
   // private void SegmentUserLearningBehavior(Guid userId, ConcurrentDictionary<Guid, FlashcardAnalytics> flashcardAnalytics, UserLearningPatterns userPatterns)
   // {
   //     double avgAccuracy = flashcardAnalytics.Values.Average(a => a.AccuracyRate);
   //     double avgTimePerCard = flashcardAnalytics.Values.Average(a => a.AverageAnswerTimeMs);
   //     double studyConsistency = userPatterns.StudyDaysInYear.Count / 365.0;
   //
   //     if (studyConsistency > 0.7 && avgAccuracy > 80)
   //         userPatterns.LearningSegment = "Consistent High Performer";
   //     else if (avgTimePerCard < 5000 && avgAccuracy > 70)
   //         userPatterns.LearningSegment = "Fast Learner";
   //     else if (studyConsistency < 0.3 && avgAccuracy < 60)
   //         userPatterns.LearningSegment = "Inconsistent Struggler";
   //     else
   //         userPatterns.LearningSegment = "Average Learner";
   // }
   //
   // private void CalculateOptimizationScore(Guid userId, ConcurrentDictionary<Guid, FlashcardAnalytics> flashcardAnalytics, UserLearningPatterns userPatterns)
   // {
   //     double totalScore = 0;
   //     int count = 0;
   //
   //     foreach (var analytics in flashcardAnalytics.Values)
   //     {
   //         double score = (analytics.AccuracyRate / 100) * 0.4 +
   //                       (1 - Math.Min(1, analytics.ForgettingIndex / 100)) * 0.3 +
   //                       Math.Min(1, 10000 / analytics.AverageAnswerTimeMs) * 0.3;
   //         analytics.OptimizationScore = score * 100;
   //         totalScore += score;
   //         count++;
   //     }
   //
   //     if (count > 0)
   //         userPatterns.AverageOptimizationScore = (totalScore / count) * 100;
   // }
   */
